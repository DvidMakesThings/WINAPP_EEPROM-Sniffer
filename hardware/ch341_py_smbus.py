...
class CH341():
    ...
    def __read_bytes(self, length=1):
        if length <= 0 or length > I2CCmd.MAX:
        ...
        try:
            rval = self.dev.read(self.EP_IN, length)
            if len(rval) != length:
                raise ConnectionError("I2C Received an incorrect number of bytes")
            return list(rval)  # Convert to list for consistency
        except usb.core.USBError as e:
            print(f"USB read error: {str(e)}")
            return None
    ...
    def read_i2c_block_data(self, addr, off, length):
        if length <= 0 or length > I2CCmd.MAX:
            print(f"Invalid read length: {length}")
            return None
